package com.ninestar.doctool.config;
...
public class WebSocketServe {
     ...
     */
    private static ConcurrentHashMap<String,WebSocketServe> webSocketSet = new ConcurrentHashMap<>();
...
//        }
     ...
     * @param message
     */
    public void GroupSendingOld(String message){
        for (String name : webSocketSet.keySet()){
            try {
                webSocketSet.get(name).session.getBasicRemote().sendText(message);
            }catch (Exception e){
